...
func TestLinkFuncAndChain(t *testing.T) {
	buf := new(bytes.Buffer)
	buf.WriteString("\n")

	decorator1 := func(next MessageFunc[string]) MessageFunc[string] {
	...
	decorator2 := func(next MessageFunc[string]) MessageFunc[string] {
		return func(dto string) error {
	...
	decorator3 := func(next MessageFunc[string]) MessageFunc[string] {
		return func(dto string) error {
	...
	tests := []struct {
		name     string
		msg      string
		decor    []MessageDecorator[string]
		expected string
	}{
		{
			name:  "Multiple decorator check sequence",
			msg:   "hello_world",
			decor: []MessageDecorator[string]{decorator1, decorator2, decorator3},
			expected: `
...
`,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := LinkFuncAndChain(baseFunc, tt.decor...)
			err := handler(tt.msg)
			if err != nil {
